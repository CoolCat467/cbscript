...
class scoreboard_var(var_base):
	...
	def is_objective(self, func, selector, objective):
		...
		if path_data is None and myselector == selector and self.objective == objective:
			return True
		else:
			return False
	...
	def uses_macro(self, func):
		return func.get_name_definition(self.selector) is not None or "$(" in self.selector
