...
def formatJsonText(func, text):
	...
	for segment, properties in parseTextFormatting(text):
		...
		if isinstance(segment, str):
			formatted = formatted + ',{{"text":"{}"{}}}'.format(segment.replace('"', '\\"'), getPropertiesText(properties))
		else:
			...
			if unformatted is None:
				...
				if command.startswith('http'):
					...
					action = 'run_command'
				formatted = formatted + ',{{"text":"{}","clickEvent":{{"action":"{}","value":"{}"}}{}}}'.format(unformatted.replace('"', '\\"'), action, command, getPropertiesText(properties))
