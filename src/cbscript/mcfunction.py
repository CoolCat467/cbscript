...
from cbscript.CompileError import CompileError
from cbscript.environment import isNumber
from cbscript.selector_definition import selector_definition
from cbscript.source_file import source_file
...
class mcfunction:
		...
		for type, val in exec_items:
			...
			if type == 'Unless':
				...
				if selector[0] == '@':
					...
					if seldef.base_name == 's' and self.environment.self_selector is not None and id in self.environment.self_selector.pointers:
	...
	def get_path(self, selector, var):
		...
		elif id == 's' and self.environment.self_selector is not None:
